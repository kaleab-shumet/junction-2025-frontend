...
export default function ConfirmationScreen() {
  ...
  const handleSendToDelivery = async () => {
    ...
    try {
      ...
      alert(
        "Your decision has been sent to the delivery team! They will update your order accordingly."
      );
      ...
      setTimeout(() => {
        if (
          confirm(
            "Would you like to see how the delivery team will view your response? Click OK to switch to delivery view."
          )
        ) {
  ...
  const replacement = replacementId
    ? mockAlternatives.find((alt) => alt.id === replacementId)
    : null;
          ...
          {originalItem && (
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-500">
                ${originalItem.originalPrice.toFixed(2)}
              </p>
            </div>
          )}
          ...
          {action === "replace" && replacement ? (
            <div className={`p-4 rounded-lg bg-blue-50 border border-blue-200`}>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {replacement.similarity}% similarity match
              </p>
